...
public class EventServiceImpl implements EventService {
    ...
    public EventDtoOut getFullEvent(Integer userId, Integer eventId) {
        checkEvent(eventId);
        return eventMapper.mapEventToEventDtoOut(eventRepository
    ...
    public List<EventShortDtoOut> getPublicEvent(String text,
        ...
        List<Event> events;
        // выборка по датам и содержанию текста
        ...
        // фильтрация по лимиту запросов на участие
        List<Integer> ids;
        if (onlyAvailable) {
            List<EventDtoOut> nextEvents = firstEvents.stream().map(eventMapper::mapEventToEventDtoOut).toList();
            ids = nextEvents.stream().filter(eventDtoOut -> eventDtoOut.getConfirmedRequests() < eventDtoOut
                            .getParticipantLimit())
    ...
    public List<EventDtoOut> getAdminEvent(Integer[] users,
        ...
        // выборка по списку id категорий и id пользователей
        ...
        if (stepThreeIds.isEmpty()) {
            if (rangeStart != null && rangeEnd != null) {
                events = eventRepository.getAdminEventByStartAndEnd(parseDate(rangeStart), parseDate(rangeEnd), from,
                        size);
            } else if (rangeStart != null && rangeEnd == null) {
    ...
    public void changeEventFields(Event event, EventUpdateDtoIn eventDtoIn) {
        ...
        if (eventDtoIn.getEventDate() != null) {
            ...
            checkValidTime(LocalDateTime.parse(eventDtoIn.getEventDate(),
                    DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")),
                    0, "Дата и время события не может быть в прошлом");
